...
if [[ ${HBASE_ROLE,,} = master ]]; then
    ...
    if [ $? != 0 ]; then
        ...
        java -jar /opt/hadoop/hdfs-fs.jar -conf $HBASE_CONF_DIR/hdfs-site.xml -chown hadoop "hdfs://${HDFS_CLUSTER_NAME}/${CLUSTER_NAME}"
    fi

    echo "Starting hbase master..."

    exec su-exec hadoop /opt/hbase/bin/hbase --config /opt/hbase/conf master start

elif [[ ${HBASE_ROLE,,} = regionserver ]]; then
    echo "Starting hbase regionserver..."

    exec su-exec hadoop /opt/hbase/bin/hbase --config /opt/hbase/conf regionserver start
